...
$guia = $_POST["eliminar"]; // Obtener el valor de "eliminar" enviado por el formulario
        ...
        $sql = "DELETE FROM devolucion WHERE fecha = ?";
...
if (mysqli_affected_rows($cone) > 0) {
    if (isset($_SESSION['nombres']) && isset($_SESSION['apellidos'])) {
        ...
        // Esta parte es para el sistema de log
        $nombreCompleto = $_SESSION['nombres'] . ' ' . $_SESSION['apellidos'];
        $python = "../python/log9999.py";
...
echo '<script>alert("Los datos se han eliminado correctamente."); window.location.href = "../html-admin/eliminar_devoluciones.php";</script>';
