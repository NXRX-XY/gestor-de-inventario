...
$validOptions = ['ctn', 'prod', 'mac', 'sn', 'cuadrilla', 'tecnico', 'fecha'];
...
if ($opcion == 'tecnico') {
  $sql = "SELECT * FROM entrega_ont WHERE tecnico1 LIKE '%$busqueda%' OR tecnico2 LIKE '%$busqueda%'";
} else {
  $sql = "SELECT * FROM entrega_ont WHERE $opcion LIKE '%$busqueda%'";
}
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_assoc($resultado)) {
        if ($fila["ctn"] != $num_guia_actual) {
            ...
            echo "<table style='border-collapse: collapse;'>";
            echo "<tr><th>Item</th><th>Ctn id</th><th>Prod id</th><th>Mac</th><th>Sn</th><th>cuadrilla</th><th>Técnico 1</th><th>Técnico 2</th><th>Fecha</th></tr>";

            $num_guia_actual = $fila["ctn"];
            $contador = 1; // Inicializar el contador en 1 para la nueva tabla
        }
   echo "<tr><td>".$contador."</td><td>".$fila["ctn"]."</td><td>".$fila["prod"]."</td><td>".$fila["mac"]."</td><td>".$fila["sn"]."</td><td>".$fila["cuadrilla"]."</td><td>".$fila["tecnico1"]."</td><td>".$fila["tecnico2"]."</td><td>".$fila["fecha"]."</td></tr>";
